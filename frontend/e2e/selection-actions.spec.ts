...
test('selected Task actions follow the status requirements', async ({page}) => {
  ...
  const cancel = page.getByRole('button', {name:'Cancel selected', exact:true});
  const requeue = page.getByRole('button', {name:'Requeue selected', exact:true});
...
test('Cancel and Requeue apply to every selected Task', async ({page}) => {
  ...
  await page.getByRole('button', {name:'Requeue selected', exact:true}).click();
  await expect(page.locator('.selection')).toHaveCount(0);
